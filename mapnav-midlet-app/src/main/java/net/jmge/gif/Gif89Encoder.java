...
public class Gif89Encoder {
   ...
   */
  public Gif89Encoder(Image static_image,boolean reducePal) throws IOException
  {
    this();
    addFrame(static_image, reducePal);
  }
   ...
   *   the limit for a GIF.
   */ 
//  public Gif89Encoder(int[] colors)
//  {
...
//  }
   ...
   *   See the addFrame() methods.   
   */ 
//  public Gif89Encoder(int[] colors, int width, int height, byte ci_pixels[])
//  throws IOException
//  {
//    this(colors);
...
//  }  
   ...
   */
  public void addFrame(Image image,boolean reducePal) throws IOException
  {
    addFrame(new DirectGif89Frame(image,reducePal));
  }
...
//  }
   ...
   */
  public void setLogicalDisplay( int background)   
  {
...
//  }
  ...
  {
    dispDim_width = gf.getWidth();
    dispDim_height = gf.getHeight();
  ...
  {
    leShort(dispDim_width, os);
    leShort(dispDim_height, os);
  ...
  private void writeCommentExtension(OutputStream os) throws IOException
  {
    ...
    for (int isb = 0; isb < nsubblocks; ++isb)
    {
      ...
      os.write(size);
      ascii(theComments.substring(ibyte, ibyte + size), os);
      ibyte += size;
    }
...
class ReverseColorMap {
  ...
  private int[] hTable_rgb = new int[HCAPACITY];
  private int[] hTable_ipalette = new int[HCAPACITY];
  private boolean[] hTable_set = new boolean[HCAPACITY];
  ...
  {      
   // ColorRecord rec;
int rec_i=-1;
    for ( int itable = rgb % hTable_rgb.length;    
          (hTable_set[itable])&& (hTable_rgb[(rec_i=itable)] != rgb);
          itable = ++itable % hTable_rgb.length
        ) 
//    for ( int itable = rgb % hTable_rgb.length;    
...
//      return rec.ipalette;
  ...
  void put(int rgb, int ipalette) 
  {
    ...
    for ( itable = rgb % hTable_set.length;    
          hTable_set[itable];
          itable = ++itable % hTable_set.length
        ) 
      ;    

    hTable_set[itable] =true;   
    hTable_rgb[itable] =rgb;   
    hTable_ipalette[itable] =ipalette;   
